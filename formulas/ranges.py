...
from .tokens.operand import Error, _index2col, _re_range, maxcol, maxrow, range2parts
...
def _merge_col_update(base, rng):
    if _has_same_sheet(base, rng) and (base["n2"] + 1) == rng["n1"]:
...
class Ranges:
    ...
    def get_range(ref, context=None, raise_anchor=True):
        ...
        try:
            ...
            raise InvalidRangeName from None
    ...
    def __add__(self, other):  # Expand.
        ...
        for r in ranges:
            if not _has_same_sheet(rng, r):
                raise InvalidRangeError(f"{self}:{other}")
            else:
    ...
    def _merge(self):
        # noinspection PyPep8
        def key(x):
            return (x["n1"], int(x["r1"]), -x["n2"], -int(x["r2"]))
    ...
    def __repr__(self):
        ...
        return f"<{self.__class__.__name__}>({ranges}){value}"
    ...
    def value(self):
        ...
        while stack:
            update = False
            for _k, (rng, value) in sorted(self.values.items()):
                if not stack:
                    break
                i = {}
                new_rng = _split(

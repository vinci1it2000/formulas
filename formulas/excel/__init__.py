...
from ..cell import Cell, CellWrapper, InvRangesAssembler, RangesAssembler, Ref
from ..errors import InvalidRangeName
from ..ranges import Ranges
from ..tokens.operand import XlError, _re_build_id, _re_sheet_id
...
def _book2dict(book):
    ...
    for ws in book.worksheets:
        ...
        for _k, cell in ws._cells.items():
...
def _file2books(*fpaths, _raw_data=False):
    ...
    return {
        osp.relpath(fp, d).upper().replace("\\", "/"): _book2dict(
    ...
    for k, v in it:
        if isinstance(v, str) and is_complex(v):
            ...
            if v.imag:
                ...
                v = v.real
        sh.get_nested_dicts(res, *k, default=lambda v=v: v)
    return res
...
class ExcelModel:
    ...
    def compare(
        ...
        **kwargs,
        ...
        if actual is None:
            ...
            actual = _convert_complex(
                _res2books(self.write(books=books, dirpath=dirpath, solution=solution))
            )
        err = diff(
            target,
            actual,
    ...
    def add_references(self, book, context=None):
        ...
        for n in it:
            ...
            ref = Ref(n.name.upper(), f"={n.value}", context).compile(context=context)
    ...
    def push(self, worksheet, context):
        ...
        for row in worksheet.iter_rows():
            for c in row:
                if hasattr(c, "value"):
                    cells.append(
                        self.compile_cell(c, ctx, references, formula_references)
                    )
        for cell in cells:
    ...
    def add_book(self, book=None, context=None, data_only=False):
        ctx = (context or {}).copy()
        _are_in, get_in = sh.are_in_nested_dicts, sh.get_nested_dicts
        ...
        if self.basedir is None:
            ...
            ctx["directory"] = ""
        if ctx["directory"]:
            ctx["directory"] = _encode_path(
                osp.relpath(
                    osp.join(self.basedir, _decode_path(ctx["directory"])), self.basedir
                )
            )
        if ctx["directory"] == ".":
            ctx["directory"] = ""
        ...
        if "external_links" not in data:
            ...
            data["external_links"] = {
                str(i + 1): osp.split(
                    osp.relpath(
                        osp.realpath(osp.join(fdir, _decode_path(el.file_link.Target))),
                        self.basedir,
                    )
                )
            ...
            data["external_links"] = {
                k: (_encode_path(d), f) for k, (d, f) in data["external_links"].items()
            }
    ...
    def add_sheet(self, worksheet, context):
        ...
        if "formula_ranges" not in d:
            d["formula_ranges"] = {
                Ranges().push(ref, context=ctx) for ref in formula_references.values()
            }
        return worksheet, ctx

    @property
    def references(self):
        return sh.combine_dicts(*(d.get("references", {}) for d in self.books.values()))
            ...
            if ref is None and data_nodes:
                for k in data_nodes:
                    if k.startswith(name):
                        for fn in self.dsp.dmap.pred.get(k) or ():
                            if not isinstance(
                                self.dsp.nodes[fn]["function"], RangesAssembler
                            ):
                                ref = k
                                break
                        break
    ...
    def anchors(self, stack=None):
        ...
        if stack is None:
            ...
            stack = {k for k in data_nodes if not pred.get(k) and k.endswith("#")}
        ...
        with tqdm.tqdm(total=len(stack)) as pbar:
            while stack:
                ...
                try:
                    ...
                    log.warning(f"Missing Reference `{n_id}`!")
                ...
                try:
                    ...
                    log.warning(f"Error in loading `{n_id}`:\n{ex}")
                    Cell(n_id, "=#REF!").compile().add(self.dsp)
                ...
                it = wk.iter_rows(
                    int(rng["r1"]),
                    min(int(rng["r2"]), max_row),
                    rng["n1"],
                    min(rng["n2"], max_column),
                )
                ctx = {"external_links": external_links}
                ctx.update(context)
                cells = []
    ...
    def _assemble_ranges(self, cells, nodes=None, compact=1):
        ...
        it = (k for k in nodes if not pred[k] and not isinstance(k, sh.Token))
        ranges = []
        for n_id in it:
            try:
            ...
            if isinstance(c, Ref) and c.inputs:
                ...
                if set(pred[inp]) == {c.func.function_id}:
                    ...
                    if not any(out in succ[k] for k in succ[inp]):
                        dsp.add_function(
                            f"={inp}", sh.bypass, inputs=[inp], outputs=[out]
                        )
                        d = nodes[inp]
                        d["inv-data"] = {out}
    ...
    def finish(self, complete=True, circular=False, assemble=True, anchors=True):
        if complete:
            self.complete()
        if anchors:
            self.anchors()
    ...
    def to_dict(self):
        ...
        nodes = {
            k: isinstance(v, str) and v.startswith("=") and f'="{v}"' or v
            for k, v in nodes.items()
        }
        nodes = {k: "#EMPTY" if v == [[sh.EMPTY]] else v for k, v in nodes.items()}
    ...
    def from_dict(self, adict, context=None, assemble=True, ref=True):
        refs, cells, nodes, _get = {}, {}, set(), sh.get_nested_dicts
    ...
    def write(self, books=None, solution=None, dirpath=None):
        ...
        for k, r in solution.items():
            ...
            if isinstance(r, Ranges):
                rng = {
                    k: v
                    for k, v in _re_sheet_id.match(r.ranges[0]["sheet_id"])
                    .groupdict()
                    .items()
                    if v is not None
                }
            ...
            if not (fpath and sheet_name):
                log.info(
                    f"Node `{k}` cannot be saved (missing filename and/or sheet_name)."
                )
                continue
            ...
            for c, v in zip(np.ravel(sheet[ref]), np.ravel(r.value)):
                try:
                    ...
                    try:
                        ...
                        c.value = ILLEGAL_CHARACTERS_RE.sub(escape_char, v)  # type: ignore[no-matching-overload]
                    if c.data_type == "f":
                        c.data_type = "s"
    ...
    def compile(self, inputs, outputs):
        ...
        dsp = dsp.get_sub_dsp_from_workflow(
            outputs, graph=dsp.dmap, reverse=True, blockers=res, wildcard=False
        )
        ...
        func = self.compile_class(
            dsp=dsp, function_id=self.dsp.name, inputs=inputs, outputs=outputs
        )

...
def _get_type_id(obj):
    if isinstance(obj, (bool, np.bool_)):
        ...
        return (
            next((i for i, k in enumerate(Error.errors.values()) if k == obj), -1) + 4
        )
...
FUNCTIONS["CHOOSE"] = wrap_ufunc(
    ...
    input_parser=lambda i, *a: (int(_convert2float(i)) - 1, np.array(a, object)),
...
def xtocol(array, ignore=0, scan_by_column=0, axis=1):
    ...
    if ignore == 0:
        ...
        def check(x):
            return x is not sh.EMPTY
    elif ignore == 2:

        def check(x):
            return not get_error(x)
    elif ignore == 3:

        def check(x):
            return not get_error(x) and x is not sh.EMPTY
    else:
        return Error.errors["#VALUE!"]
...
FUNCTIONS["_XLFN.TOCOL"] = FUNCTIONS["TOCOL"] = wrap_ufunc(
    ...
    input_parser=lambda array, ignore=0, scan_by_column=0: (
        array,
        int(_convert2float(ignore)),
        int(scan_by_column),
    ),
    args_parser=lambda array, ignore=0, scan_by_column=0: (
        array,
        replace_empty(ignore),
        replace_empty(scan_by_column),
    ),
...
def return_unique_func(res, *args):
    ...
    res[res == None] = Error.errors["#VALUE!"]  # noqa: E711
...
FUNCTIONS["_XLFN.UNIQUE"] = FUNCTIONS["UNIQUE"] = wrap_ufunc(
    ...
    args_parser=lambda array, by_col=0, exactly_once=0: (
        array,
        replace_empty(by_col),
        replace_empty(exactly_once),
    ),
...
def xsortby(ref, by_array, sort_order1=1, *args):
    ...
    if (
        1 not in shape
        or any(shape != a.shape for a in args[::2])
...
def map_multiindex_take_last_if_tuple(mi):
    ...
    if isinstance(mi, pd.MultiIndex):
        new_tuples = [tuple(_last_if_tuple(v) for v in tup) for tup in mi.to_list()]
...
def _xpivotby(
    ...
    aggfunc,
    field_headers=None,
    row_total_depth=None,
    row_sort_order=1,
    col_total_depth=None,
    col_sort_order=1,
    filter_array=None,
    relative_to=0,
    _groupby=False,
    ...
    if field_headers is None:
        ...
        if (
            row_fields.shape[1] > 1 or values.shape[1] > 1 or col_fields.shape[1] > 1
        ) and not _groupby:
            field_headers += 2
    ...
    try:
        ...
        def agg(s, *args):
            return "" if (s == "").all() else to_python(aggfunc(s))
    except Exception:
        if get_error(to_python(aggfunc([1, 1], [1, 1]))):

            def agg(s, *args):
                return "" if (s == "").all() else Error.errors["#VALUE!"]
        else:

            def agg(s, *args):
                return "" if (s == "").all() else to_python(aggfunc(s, *args))
    ...
    for k, it in {
        ...
        for i, v in enumerate(it.T, 1):
            ...
            sh.get_nested_dicts(pivot, k, default=collections.OrderedDict)[i] = j
    ...
    elif len(abs_col) != col_sort_order.size or abs_col - set(range(1, len(cols) + 1)):
        raise FoundError(err=Error.errors["#VALUE!"])
    else:
        col_sort_order = {cols[i] for i in (-col_sort_order[col_sort_order < 0] - 1)}
    ...
    elif len(abs_row) != row_sort_order.size or abs_row - set(range(1, len(rows) + 1)):
        raise FoundError(err=Error.errors["#VALUE!"])
    else:
        row_sort_order = {
            rows[i] for i in (-row_sort_order[row_sort_order < 0] - 1).astype(int)
        }
    for value in values:
        if add_totalset:
                    ...
                    for j in range(0, len(rows) + 1):
                        ...
                        if keys:
                            for k, v in dat.groupby(keys).groups.items():
                                ...
                                _totalset[(k[:i], k[i:])] = dat.loc[v, value].values
                        else:
        ...
        it_c = sorted(set(list(range(0, int(abs(col_total_depth)))) + [len(cols)]))
        it_r = sorted(set(list(range(0, int(abs(row_total_depth)))) + [len(rows)]))
        for i in it_c:
            if add_totalset and relative_to == 0 and i:  # 0: Column Totals(Default)
                _totalset = {
                    k: v[[value]].values
                    for k, v in data[[value] + cols[:i]].groupby(cols[:i])
                }
                _totalset[()] = data[[value]].values
            for j in it_r:
                if add_totalset and relative_to == 1 and j:  # 1: Row Totals
                    _totalset = {
                        k: v[[value]].values
                        for k, v in data[[value] + rows[:j]].groupby(rows[:j])
                    }
                    _totalset[()] = data[[value]].values
                ...
                if keys:
                    it = (
                        ((k,) if len(keys) == 1 else k, dat.loc[v, value].values)
                        for k, v in dat.groupby(keys).groups.items()
                    )
                else:
                    it = [((), dat.values)]
                for k, v in it:
                    if add_totalset:
                        if relative_to == 0:
                            ...
                            totalset = _totalset[(k[:i], (k[i:-1] if j > 1 else ()))]

                    v = agg(v, totalset)
                    df.append({"Value": value, "agg": v, **sh.map_list(keys, *k)})
    ...
    for i in rows:
        ...
        df[i] = df[i].map(lambda x, tot_key=tot_key: tot_key if pd.isna(x) else x)
    ...
    if sort_row_by_value:
        b = np.asarray([abs(x[0]) == 50 for x in df.columns.get_level_values(cols[0])])
        df["sort"] = [
            (i[0] if abs(i[0]) == 50 else 0, v, i)
            for v, i in zip(
                df.iloc[:, b].values.ravel(), df.index.get_level_values(rows[-1])
            )
        ]
        df.sort_values(
            by=rows[:-1] + ["sort"],
            ascending=[k not in row_sort_order for k in rows],
            axis=0,
            inplace=True,
        )
    ...
    if sort_col_by_value:
        b = np.asarray([abs(x[0]) == 50 for x in df.index.get_level_values(rows[0])])
        v = [
            (i[0] if abs(i[0]) == 50 else 0, v, i)
            for v, i in zip(
                df.iloc[b].values.ravel(), df.columns.get_level_values(cols[-1])
            )
        ]
        df = df.T
        df["sort"] = v
        df = df.T
        df.sort_values(
            by=cols[:-1] + ["sort", "Value"],
            ascending=[k not in col_sort_order for k in cols + ["Value"]],
            axis=1,
            inplace=True,
        )
    ...
    columns_names = [pivot["Column Field"].get(k, k) for k in df.columns.names]
...
FUNCTIONS["_XLFN.PIVOTBY"] = FUNCTIONS["PIVOTBY"] = wrap_func(
    functools.partial(_xpivotby, _groupby=False)
)
...
def xgroupby(
    ...
    return _xpivotby(
        ...
        row_total_depth=row_total_depth,
...
FUNCTIONS["_XLFN.TRIMRANGE"] = FUNCTIONS["TRIMRANGE"] = wrap_ufunc(
    xtrimrange,
    input_parser=lambda arr, trim_rows, trim_cols: (
        arr,
        _convert2float(trim_rows),
        _convert2float(trim_cols),
    ),
    check_error=lambda arr, trim_rows=0, trim_cols=0: get_error(trim_rows, trim_cols),
    args_parser=lambda arr, trim_rows=0, trim_cols=0: (
        np.atleast_2d(arr),
        replace_empty(trim_rows),
        replace_empty(trim_cols),
    ),
    return_func=return_trimrange_func,
    check_nan=False,
    excluded={0},
)
...
FUNCTIONS["_XLFN.EXPAND"] = FUNCTIONS["EXPAND"] = wrap_ufunc(
    xexpand,
    input_parser=lambda arr, rows, columns, pad_with: (
        arr,
        int(_convert2float(rows)),
        columns,
        pad_with,
    ),
...
def xwrapcols(array, wrap_count, pad_with, cols=True):
    ...
    if need:
        array = np.pad(array, (0, need), mode="constant", constant_values=pad_with)
...
FUNCTIONS["_XLFN.WRAPROWS"] = FUNCTIONS["WRAPROWS"] = wrap_ufunc(
    ...
    args_parser=lambda arr, wrap_count, pad_with=Error.errors["#N/A"]: (
        np.asarray(arr, object).ravel(),
        replace_empty(wrap_count),
        pad_with,
    ),
    return_func=return_2d_func,
    check_nan=False,
...
def _index(arrays, row_num, col_num, area_num, is_reference, is_array):
    ...
    try:
        ...
        val = array[row_num, col_num]  # type: ignore[not-subscriptable]
            ...
            for rng in array.ranges:
                ...
                for r in range(int(rng["r1"]), int(rng["r2"]) + 1):
                    ...
                    for n in range(rng["n1"], rng["n2"] + 1):
                        ...
                        name = _name % ref
                        row.append(
                            Ranges(values=dict(array.values)).set_value(
                                {
                                    "r1": r,
                                    "r2": r,
                                    "c1": c,
...
def xxmatch(
    ...
    if match_type == -1:
        ...
        def stop(i, x, force_update=False):
            if x[0] == lookup_value_type and match(x[1]):  # type: ignore[call-non-callable]
                best[0], best[1] = i, x
                return True
            return False
    else:
...
def args_parser_match_array(val, arr, match_type=1, lower=_lower):
    ...
    return (
        ...
        lookup_array,
        lookup_array_raw,
        next(flatten([match_type], None)),
    )


def args_parser_xmatch_array(val, arr, match_mode=0, search_mode=1):
    (val_types, val, val_raw, index, arr_types, lookup_array, lookup_array_raw) = (
        args_parser_match_array(replace_empty(val, None), arr, lower=_casefold)[:-1]
    )
...
def input_parser_xmatch(
    ...
    if (
        not (-2 < match_mode <= 3)
        or (abs(match_mode) == 2 and abs(search_mode) == 2)
...
FUNCTIONS["MATCH"] = wrap_ufunc(
    ...
    excluded={3, 4, 5, 6, 7},
    args_parser=args_parser_match_array,
    input_parser=lambda *a: a,
)
...
FUNCTIONS["_XLFN.XMATCH"] = FUNCTIONS["XMATCH"] = wrap_ufunc(
    ...
    check_error=lambda *a: get_error(a[1]),
    excluded={3, 4, 5, 6},
    args_parser=args_parser_xmatch_array,
    input_parser=input_parser_xmatch,
)
...
def xfilter(array, condition, if_empty=Error.errors["#VALUE!"]):
    ...
    c_shp = b.shape or (1,)
    if not (
        (len(c_shp) == 1 or (len(c_shp) == 2 and 1 in c_shp)) and 1 <= len(a_shp) <= 2
    ):
        return Error.errors["#VALUE!"]
    b = b.ravel()
...
def args_parser_lookup_array(lookup_val, lookup_vec, result_vec=None, match_type=1):
    result_vec = np.ravel(lookup_vec if result_vec is None else result_vec)
    return args_parser_match_array(lookup_val, lookup_vec, match_type) + (result_vec,)
...
def xlookup(
    ...
    if not isinstance(r, XlError):
        r = np.asarray(result_vec[r - 1], object).ravel()[0]  # type: ignore[not-subscriptable]
    return r
...
def args_parser_xlookup_array(
    ...
    return args_parser_xmatch_array(lookup_val, lookup_vec, match_mode, search_mode) + (
        if_not_found,
        np.ravel(return_array),
    )
...
def xxlookup(
    ...
    if isinstance(r, XlError):
        ...
        r = np.asarray(return_array[r - 1], object).ravel()[0]  # type: ignore[not-subscriptable]
...
FUNCTIONS["_XLFN.XLOOKUP"] = FUNCTIONS["XLOOKUP"] = wrap_ufunc(
    ...
    check_error=lambda *a: get_error(a[1]),
    excluded={3, 4, 5, 6, 10},
    args_parser=args_parser_xlookup_array,
    input_parser=input_parser_xlookup,
)
...
def args_parser_hlookup(val, vec, index, match_type=1, transpose=False):
    ...
    try:
        ...
        raise FoundError(err=Error.errors["#REF!"]) from None
    vec = vec[0].ravel()

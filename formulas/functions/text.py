...
def xnumbervalue(text, decimal_sep=None, group_sep=None):
    ...
    if {"(", ")"}.intersection({g, d}):
        ...
        m = re.match(f"(?:^\\s*[+-]?{number}$)|(?:^\\s*\\({number}\\)[\\s%]*$)", text)
    if not m:
        return Error.errors["#VALUE!"]
    return float(
        text.translate(
...
def _xregex(text, pattern, case_sensitivity=0):
    ...
    rx = re.compile(_str(pattern), re.IGNORECASE if case_sensitivity == 1 else 0)
    return rx, _str(text)


_get_first = np.vectorize(lambda x: x[0] if isinstance(x, list) else x, otypes=[object])
...
def xregexreplace(text, pattern, replacement, occurrence=0, case_sensitivity=0):
    rx, text = _xregex(text, pattern, case_sensitivity)
    return rx.sub(_str(replacement), text, count=int(float(replace_empty(occurrence))))
...
FUNCTIONS["REGEXEXTRACT"] = FUNCTIONS["_XLFN.REGEXEXTRACT"] = wrap_ufunc(
    xregexextract, return_func=_xregexextract_return_func, check_nan=False, **_kw0
)
...
def xsubstitute(text, old_text, new_text, instance_num=None):
    ...
    elif isinstance(instance_num, (bool, np.bool_, str, np.str_)) or instance_num < 1:
        return Error.errors["#VALUE!"]
    ...
    return (
        old_text.join(parts[:instance_num])
        + new_text
        + old_text.join(parts[instance_num:])
    )
...
FUNCTIONS["_XLFN.CONCATENATE"] = FUNCTIONS["CONCATENATE"] = wrap_ufunc(xconcat, **_kw0)
...
def _parse_format_code(format_code):
        ...
        for k, v in match.groupdict().items():
            if v:
                if k == "decimal" and k in types:
                    ...
                    if suspended:
                        sh.get_nested_dicts(
                            types, "s" in v.lower() and k or "date", default=list
    ...
    if len(suspended) == 1:
        sh.get_nested_dicts(types, "date", default=list).append(suspended.pop())
    code.extend(codes)
    ...
    for condition, codes, types in formats:
        ...
        if condition:
            ...
            check = functools.partial(LOGIC_OPERATORS[operator], y=_text2num(condition))
        else:
...
def xvalue(value):
    if not isinstance(value, Error) and isinstance(value, str):
        try:
            ...
            from .date import _text2datetime, xdate, xtime
...
def xarraytotext(array, format_type=0):
    ...
    if format_type == 0:
        return ", ".join(_xvaluetotext(v, format_type) for v in flatten(array, None))
    elif format_type == 1:
        array = np.atleast_2d(array)
        if len(array.shape) == 2:
            r = ";".join(
                ",".join(_xvaluetotext(v, format_type) for v in row) for row in array
            )
...
def xtextsplit(
    ...
    if row_delimiter is None:
        ...
        if not row_delimiter or any(not v for v in row_delimiter):
            return Error.errors["#VALUE!"]
        rows = re.split("|".join(map(re.escape, row_delimiter)), text, flags=flags)
...
def xtextafterbefore(
    ...
    r = list(re.finditer("|".join(map(re.escape, delimiter)), text, flags=flags))
    try:

...
def _xskew(bias, v):
    ...
    return float(stats.skew(v, bias=bias, nan_policy="omit"))


xskewp = functools.partial(xfunc, func=functools.partial(_xskew, True), default=None)
FUNCTIONS["_XLFN.SKEW.P"] = FUNCTIONS["SKEW.P"] = wrap_func(xskewp)
xskew = functools.partial(xfunc, func=functools.partial(_xskew, False), default=None)
FUNCTIONS["_XLFN.SKEW"] = FUNCTIONS["SKEW"] = wrap_func(xskew)
xmode_sngl = functools.partial(
    xfunc,
    func=_xmode_sngl,
    convert=_convert,
    parse_args=lambda a: (v for v in a if v is not ""),  # noqa: F632
)
...
FUNCTIONS["COUNTA"] = wrap_func(
    functools.partial(xfunc, check=is_not_empty, func=len, _raise=False, default=None)
)
FUNCTIONS["COUNTBLANK"] = wrap_func(
    functools.partial(
        xfunc,
        check=lambda x: (x == "" or x is sh.EMPTY),
        func=len,
        _raise=False,
        default=None,
    )
)
FUNCTIONS["COUNTIF"] = wrap_func(functools.partial(xfilter, len, operating_range=None))
FUNCTIONS["COUNTIFS"] = wrap_func(functools.partial(xfilters, len, None))
...
def _sort_parser(values, k):
    ...
    if err:
        return err, k
    values = np.array(
        tuple(flatten(values, lambda v: not isinstance(v, (str, bool)))), float
    )
    values.sort()
...
FUNCTIONS["MAXA"] = wrap_func(
    functools.partial(xfunc, convert=_convert, check=is_not_empty)
)
FUNCTIONS["_XLFN.MAXIFS"] = FUNCTIONS["MAXIFS"] = wrap_func(
    functools.partial(xfilters, xfunc)
)

FUNCTIONS["MEDIAN"] = wrap_func(
    functools.partial(
        xfunc, func=lambda x: convert_nan(np.median(x) if x else np.nan), default=None
    )
)
FUNCTIONS["MIN"] = wrap_func(functools.partial(xfunc, func=min))
FUNCTIONS["MINA"] = wrap_func(
    functools.partial(xfunc, convert=_convert, check=is_not_empty, func=min)
)
FUNCTIONS["_XLFN.MINIFS"] = FUNCTIONS["MINIFS"] = wrap_func(
    functools.partial(xfilters, functools.partial(xfunc, func=min))
)
...
def _prepare_ets_data(values, timeline, data_completion, aggregation):
    ...
    if b.any():
        if data_completion:
            y[b] = interp1d(x[~b], y[~b], kind="linear", fill_value="extrapolate")(x[b])
        else:
            y = np.nan_to_num(y, False)
    return x, y, step


def _train_ets_model(values, timeline, seasonality, data_completion, aggregation):
    x, y, step = _prepare_ets_data(values, timeline, data_completion, aggregation)
    ...
    model = ETSModel(pd.Series(y), trend="add", **kw).fit(warn_convergence=False)
    return model, x, y, step


def xforecast_ets(
    target_date, values, timeline, seasonality=1, data_completion=1, aggregation=0
):
    ...
    return float(
        interp1d(x, pred.predicted_mean, kind="linear", fill_value="extrapolate")(
            target_date
        )
    )
...
def xforecast_ets_confint(
    ...
    pred = model.get_prediction(start=0, end=h)
    var = float(
        interp1d(x, pred.forecast_variance, kind="linear", fill_value="extrapolate")(
            target_date
        )
    )
...
def xforecast_ets_stat(
    values, timeline, statistic_type, seasonality=1, data_completion=1, aggregation=0
):
    model, x, y, step = _train_ets_model(
        values, timeline, seasonality, data_completion, aggregation
    )
    ...
    if statistic_type == 4:
        ...
        if np.isclose(num, 0, atol=1.0e-7):
            return 0
...
def xforecast_ets_seasonality(values, timeline, data_completion=1, aggregation=0):
    model = _train_ets_model(values, timeline, 1, data_completion, aggregation)[0]
...
FUNCTIONS["_XLFN.FORECAST.LINEAR"] = FUNCTIONS["FORECAST"] = wrap_ufunc(
    xforecast,
    args_parser=_args_parser_forecast,
    excluded={1, 2},
    input_parser=lambda x, a, b: (_convert_args(x), a, b),
)
FUNCTIONS["FORECAST.LINEAR"] = FUNCTIONS["FORECAST"]
FUNCTIONS["_XLFN.FORECAST.ETS.STAT"] = FUNCTIONS["FORECAST.ETS.STAT"] = wrap_ufunc(
    xforecast_ets_stat,
    excluded={0, 1},
    check_error=lambda values,
    timeline,
    statistic_type,
    seasonality=1,
    data_completion=1,
    aggregation=0: get_error(statistic_type, seasonality, data_completion, aggregation),
    args_parser=lambda values,
    timeline,
    statistic_type,
    seasonality=1,
    data_completion=1,
    ...
    aggregation=0: (
        values,
        timeline,
        _convert_args(statistic_type),
        _convert_args(seasonality),
        _convert_args(data_completion),
        _convert_args(aggregation),
    ),
)
FUNCTIONS["_XLFN.FORECAST.ETS.SEASONALITY"] = FUNCTIONS["FORECAST.ETS.SEASONALITY"] = (
    wrap_ufunc(
        ...
        timeline,
        data_completion=1,
        aggregation=0: get_error(data_completion, aggregation),
        args_parser=lambda values, timeline, data_completion=1, aggregation=0: (
            replace_empty(values, ""),
            replace_empty(timeline, ""),
            replace_empty(data_completion, 1),
            replace_empty(aggregation),
        ),
        input_parser=lambda values, timeline, data_completion=1, aggregation=0: (
            values,
            timeline,
            _convert_args(data_completion),
            _convert_args(aggregation),
...
FUNCTIONS["_XLFN.FORECAST.ETS.CONFINT"] = FUNCTIONS["FORECAST.ETS.CONFINT"] = (
    wrap_ufunc(
        xforecast_ets_confint,
        excluded={1, 2},
        check_error=lambda target_date,
        values,
        timeline,
        confidence_level=0.95,
        seasonality=1,
        data_completion=1,
        aggregation=0: get_error(
            target_date, confidence_level, seasonality, data_completion, aggregation
        ),
        args_parser=lambda target_date,
        values,
        timeline,
        confidence_level=0.95,
        seasonality=1,
        ...
        input_parser=lambda target_date,
        values,
        timeline,
        confidence_level=0.95,
        seasonality=1,
        data_completion=1,
        aggregation=0: (
            _convert_args(target_date),
            values,
            timeline,
            _convert_args(confidence_level),
            _convert_args(seasonality),
            _convert_args(data_completion),
            _convert_args(aggregation),
        ),
...
FUNCTIONS["_XLFN.FORECAST.ETS"] = FUNCTIONS["FORECAST.ETS"] = wrap_ufunc(
    ...
    seasonality=1,
    data_completion=1,
    aggregation=0: get_error(target_date, seasonality, data_completion, aggregation),
    args_parser=lambda target_date,
    values,
    timeline,
    seasonality=1,
...
FUNCTIONS["_XLFN.NORM.INV"] = FUNCTIONS["NORM.INV"] = wrap_ufunc(
    xnorminv, input_parser=lambda *a: tuple(map(_convert_args, a))
)
...
FUNCTIONS["_XLFN.NORM.S.INV"] = FUNCTIONS["NORM.S.INV"] = wrap_ufunc(
    xnorminv, input_parser=lambda x: (_convert_args(x),)
)
...
FUNCTIONS["_XLFN.LOGNORM.INV"] = FUNCTIONS["LOGNORM.INV"] = wrap_ufunc(
    xlognorminv, input_parser=lambda *a: tuple(map(_convert_args, a))
)
...
FUNCTIONS["_XLFN.BETA.DIST"] = FUNCTIONS["BETA.DIST"] = wrap_ufunc(
    ...
    + tuple(map(_parse_cumulative, a[3:4]))
    + tuple(map(_convert_args, a[4:])),
)
FUNCTIONS["_XLFN.BETA.INV"] = FUNCTIONS["BETA.INV"] = wrap_ufunc(
    xbetainv, input_parser=lambda *a: tuple(map(_convert_args, a))
)
...
def xbinomdistrange(trials, probability_s, number_s, number_s2=None):
    ...
    if not (
        0 <= probability_s <= 1
        and (0 <= number_s <= trials and 0 <= number_s2 <= trials)
    ):
        return Error.errors["#NUM!"]
...
FUNCTIONS["BINOM.DIST.RANGE"] = wrap_ufunc(
    xbinomdistrange, input_parser=lambda *a: tuple(map(_convert_args, a))
)
FUNCTIONS["_XLFN.BINOM.DIST.RANGE"] = FUNCTIONS["BINOM.DIST.RANGE"]
FUNCTIONS["_XLFN.BINOM.INV"] = FUNCTIONS["BINOM.INV"] = wrap_ufunc(
    xbinominv, input_parser=lambda *a: tuple(map(_convert_args, a))
)
...
FUNCTIONS["_XLFN.CHISQ.INV"] = FUNCTIONS["CHISQ.INV"] = wrap_ufunc(
    xchisqinv, input_parser=lambda *a: tuple(map(_convert_args, a))
)
FUNCTIONS["_XLFN.CHISQ.DIST.RT"] = FUNCTIONS["CHISQ.DIST.RT"] = wrap_ufunc(
    xchisqdistrt, input_parser=lambda *a: tuple(map(_convert_args, a))
)
FUNCTIONS["_XLFN.CHISQ.INV.RT"] = FUNCTIONS["CHISQ.INV.RT"] = wrap_ufunc(
    xchisqinvrt, input_parser=lambda *a: tuple(map(_convert_args, a))
)
FUNCTIONS["_XLFN.CHISQ.TEST"] = FUNCTIONS["CHISQ.TEST"] = wrap_func(xchisqtest)
...
FUNCTIONS["_XLFN.CONFIDENCE.NORM"] = FUNCTIONS["CONFIDENCE.NORM"] = wrap_ufunc(
    xconfidence_norm, input_parser=lambda *a: tuple(map(_convert_args, a))
)
FUNCTIONS["_XLFN.CONFIDENCE.T"] = FUNCTIONS["CONFIDENCE.T"] = wrap_ufunc(
    xconfidence_t, input_parser=lambda *a: tuple(map(_convert_args, a))
)
...
FUNCTIONS["_XLFN.COVARIANCE.P"] = FUNCTIONS["COVARIANCE.P"] = wrap_func(xcovariance_p)
FUNCTIONS["_XLFN.COVARIANCE.S"] = FUNCTIONS["COVARIANCE.S"] = wrap_func(xcovariance_s)
...
def xt_inv(probability, deg_freedom):
    deg_freedom = int(deg_freedom)
    if not (0 <= probability <= 1 and deg_freedom >= 1):
        return Error.errors["#NUM!"]
...
def xt_inv2t(probability, deg_freedom):
    ...
    if not (0 <= probability <= 1 and deg_freedom >= 1):
        return Error.errors["#NUM!"]
    return stats.t.ppf(1.0 - probability / 2.0, deg_freedom)
...
FUNCTIONS["_XLFN.T.INV"] = FUNCTIONS["T.INV"] = wrap_ufunc(
    xt_inv, input_parser=lambda *a: tuple(map(_convert_args, a))
)
FUNCTIONS["_XLFN.T.DIST.2T"] = FUNCTIONS["T.DIST.2T"] = wrap_ufunc(
    xt_dist2t, input_parser=lambda *a: tuple(map(_convert_args, a))
)
FUNCTIONS["_XLFN.T.DIST.RT"] = FUNCTIONS["T.DIST.RT"] = wrap_ufunc(
    xt_distrt, input_parser=lambda *a: tuple(map(_convert_args, a))
)
FUNCTIONS["_XLFN.T.INV.2T"] = FUNCTIONS["T.INV.2T"] = wrap_ufunc(
    xt_inv2t, input_parser=lambda *a: tuple(map(_convert_args, a))
)
...
FUNCTIONS["_XLFN.EXPON.DIST"] = FUNCTIONS["EXPON.DIST"] = wrap_ufunc(
    xexpon_dist,
    input_parser=lambda *a: tuple(map(_convert_args, a[:-1]))
...
FUNCTIONS["FISHER"] = wrap_ufunc(xfisher, input_parser=lambda x: (_convert_args(x),))
FUNCTIONS["FISHERINV"] = wrap_ufunc(
    xfisherinv, input_parser=lambda y: (_convert_args(y),)
)
...
FUNCTIONS["_XLFN.GAMMA"] = FUNCTIONS["GAMMA"] = wrap_ufunc(
    xgamma, input_parser=lambda x: (_convert_args(x),)
)
...
FUNCTIONS["_XLFN.GAUSS"] = FUNCTIONS["GAUSS"] = wrap_ufunc(
    xgauss, input_parser=lambda x: (_convert_args(x),)
)


def xhypergeom_dist(sample_s, number_sample, population_s, number_pop, cumulative):
    """
    EXPON.DIST(x, lambda, cumulative)
      - cumulative TRUE  -> 1 - exp(-lambda * x)
      - cumulative FALSE -> lambda * exp(-lambda * x)
...
_percentrank_kw = {
    ...
    "args_parser": lambda v, q, s=3: (
        sorted(_parse_ranges(v, [], error_x_row=False, raise_diff_len=False)[0]),
        replace_empty(q),
        replace_empty(s),
    ),
}
...
_percentile_kw = {
    ...
    "check_error": lambda *a: get_error(*a[::-1]),
    "args_parser": lambda v, q: (list(flatten(v, drop_empty=True)), replace_empty(q)),
}
...
_quartile_kw = sh.combine_dicts(
    _percentile_kw,
    {
        ...
        "args_parser": lambda v, q: (
            list(flatten(v, drop_empty=True)),
            replace_empty(q),
        ),
    },
)
...
FUNCTIONS["_XLFN.STDEV.S"] = FUNCTIONS["STDEV.S"] = wrap_func(
    functools.partial(xfunc, func=xstdev)
)
FUNCTIONS["_XLFN.STDEV.P"] = FUNCTIONS["STDEV.P"] = wrap_func(
    functools.partial(xfunc, func=functools.partial(xstdev, ddof=0), default=None)
)
FUNCTIONS["STDEVA"] = wrap_func(
    functools.partial(xfunc, convert=_convert, check=is_not_empty, func=xstdev)
)
FUNCTIONS["STDEVPA"] = wrap_func(
    functools.partial(
        xfunc,
        convert=_convert,
        check=is_not_empty,
        func=functools.partial(xstdev, ddof=0),
        default=None,
    )
)

FUNCTIONS["_XLFN.VAR.S"] = FUNCTIONS["VAR.S"] = wrap_func(
    functools.partial(xfunc, func=functools.partial(xstdev, func=np.var))
)
FUNCTIONS["_XLFN.VAR.P"] = FUNCTIONS["VAR.P"] = wrap_func(
    functools.partial(
        xfunc, func=functools.partial(xstdev, ddof=0, func=np.var), default=None
    )
)
FUNCTIONS["VARA"] = wrap_func(
    functools.partial(
        xfunc,
        convert=_convert,
        check=is_not_empty,
        func=functools.partial(xstdev, func=np.var),
    )
)
FUNCTIONS["VARPA"] = wrap_func(
    functools.partial(
        xfunc,
        convert=_convert,
        check=is_not_empty,
        func=functools.partial(xstdev, ddof=0, func=np.var),
        default=None,
    )
)
...
FUNCTIONS["_XLFN.PERMUTATIONA"] = FUNCTIONS["PERMUTATIONA"] = wrap_ufunc(xpermutationa)
...
FUNCTIONS["_XLFN.RANK.EQ"] = FUNCTIONS["RANK.EQ"] = wrap_ufunc(
    ...
    args_parser=lambda number, ref, order=0: (
        replace_empty(number),
        ref,
        replace_empty(order),
    ),
    input_parser=lambda number, ref, order=0: (
        _convert_args(number),
        sorted(_parse_ranges(ref, [], error_x_row=False, raise_diff_len=False)[0]),
        _convert_args(order),
    ),
)
FUNCTIONS["_XLFN.RANK.AVG"] = FUNCTIONS["RANK.AVG"] = wrap_ufunc(
    ...
    input_parser=lambda number, ref, order=0: (
        _convert_args(number),
        _parse_ranges(ref, [], error_x_row=False, raise_diff_len=False)[0],
        _convert_args(order),
    ),
)
...
FUNCTIONS["TRIMMEAN"] = wrap_ufunc(
    ...
    args_parser=lambda array, percent: (
        np.sort(
            np.array(
                _parse_ranges(array, [], error_x_row=False, raise_diff_len=False)[0],
                dtype=float,
            )
        ),
        replace_empty(percent),
    ),
...
FUNCTIONS["_XLFN.Z.TEST"] = FUNCTIONS["Z.TEST"] = wrap_ufunc(
    xz_test,
    excluded={0},
    check_error=lambda array, x, sigma=None: get_error(x)
    or get_error(sigma)
    or get_error(array),
    args_parser=lambda array, x, sigma=None: (
        array,
        replace_empty(x),
        replace_empty(sigma, None),
    ),
    input_parser=lambda array, x, sigma: (
        array,
        float(_convert_args(x)),
        _convert_args(sigma),
    ),
)
...
def xfrequency(data_array, bins_array):
    ...
    x = np.array(
        _parse_ranges(data_array, [], error_x_row=False, raise_diff_len=False)[0],
        dtype=float,
    )
    bins = np.array(
        _parse_ranges(bins_array, [], error_x_row=False, raise_diff_len=False)[0]
        or [0],
        dtype=float,
    )
...
def _parse_linest(known_y, known_x=None, new_x=None):
    ...
    if (
        ...
        or not (
            ...
            or (y.shape[1] == 1 and y.shape[0] == x.shape[0])
        )
        or not (
            (y.shape[0] == 1 and x.shape[0] == _new_x.shape[0])
            or (y.shape[1] == 1 and x.shape[1] == _new_x.shape[1])
        )
    ):
...
def xlinest(known_y, known_x=None, const=True, _stats=False):
    x, y, const, _stats = _xlinest_parse(known_y, known_x, const, _stats)[:-1]
...
def xtrend(known_y, known_x=None, new_x=None, const=True):
    x, y, const, _stats, new_x = _xlinest_parse(known_y, known_x, const, False, new_x)
    if const:

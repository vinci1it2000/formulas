...
from ..errors import FormulaError, FoundError, TokenError
from . import Token
...
class Function(Token):
    ...
    def process(self, match, context=None, parser=None):
        attr = super().process(match, context, parser)
    ...
    def set_expr(self, *tokens):
        ...
        self.attr["expr"] = f"{self.name.upper()}({args})"


_re_lambda = regex.compile(
    r"""
...
class LambdaFunction(functools.partial):
    def __new__(cls, __func, *args, **kwargs):
        return super().__new__(cls, wrap_func(__func), *args, **kwargs)
...
class Lambda(Function):
    ...
    def _process(self, attr, match, context=None, parser=None):
        ...
        ext_inp.update((t.name, t) for t in calc_tkns if isinstance(t, Range))
        for t in vars:
        ...
        attr["func"] = {
            "expr": "".join(expr),
            "external_inputs": collections.OrderedDict(sorted(ext_inp.items())),
            "tokens": tokens,
            "calculation": calc,
    ...
    def compile(self):
        if self.has_leta:
            ...
            if isinstance(func, dict):
                func = func.copy()
                func["function"] = functools.partial(LetaFunction, func["function"])
                return func

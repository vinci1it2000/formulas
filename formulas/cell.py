...
from .functions import DSP, replace_empty
from .parser import Parser
from .ranges import Ranges, _assemble_values, _get_indices_intersection, _shape
from .tokens.operand import Error, XlError, _index2col, _re_ref, range2parts
...
class Cell:
    ...
    def __init__(
        ...
        if reference is not None:
            ...
            context = sh.combine_dicts(
                context or {},
                sh.selector(("filename", "sheet"), r, allow_miss=True),
    ...
    def _args(self, *args):
        assert self.inputs is not None and len(args) == len(self.inputs)
        inputs = {
...
class Ref(Cell):
    def __init__(self, reference, value, context=None, check_formula=True):
        ...
        context.update(
            {
                k: v
                for k, v in _re_ref.match(reference).groupdict().items()
                if v is not None
            }
        )
        ref = context.pop("ref")
        super().__init__(None, value, context, check_formula)
    ...
    def compile(self, references=None, context=None):
        super().compile(references=references, context=context)
        if self.inputs:
...
class RangesAssembler:
    ...
    def push(self, indices, output=None):
        ...
        if it:
            ...
            it = {indices[i]: i for i in it} if output is None else {output: indices}
    ...
    def add(self, dsp):
        ...
        if list(self.inputs) != [self.output]:
            ...
            if len(self.outputs) >= 1:
                ...
                dsp.add_function(None, InvRangesAssembler(self), inputs, self.outputs)

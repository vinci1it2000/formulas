...
class TestExcelModel(unittest.TestCase):
    def setUp(self):
        ...
        self.results = _convert_complex(
            _file2books(self.filename, self.link_filename, _raw_data=True)
        )
        self.results_ods = _convert_complex(
            _file2books(self.filename_ods, self.link_filename_ods, _raw_data=True)
        )
        ...
        self.results_circular = _convert_complex(
            _file2books(self.filename_circular, _raw_data=True)
        )
        self.results_full_range = {
            "TEST_FILES/%s" % k: v
            for k, v in _convert_complex(
                _file2books(self.filename_full_range, _raw_data=True)
            ).items()
        }
        sh.get_nested_dicts(
    ...
    def _compare(self, xl_mdl, target, **kw):
        errors = xl_mdl.compare(target=target, absolute_tolerance=0.0001, **kw)
    ...
    def test_ods_model(self):
        ...
        msg = "%sCalculated ods-model in %.2fs.\n%sComparing overwritten results."
        print(msg % (_msg, time.time() - s, _msg))
        s = time.time()

        n_test += self._compare(
            xl_mdl, self.results_ods, books=xl_mdl.books, solution=xl_mdl.dsp.solution
        )

        msg = (
            "%sCompared overwritten results in %.2fs.\n"
            "%sComparing fresh written results."
        )
        print(msg % (_msg, time.time() - s, _msg))
        s = time.time()

        n_test += self._compare(xl_mdl, self.results_ods, solution=xl_mdl.dsp.solution)
    ...
    @unittest.skipIf(
        ...
        "schedula deepcopy incompatible with Python 3.14+ (itertools.count)",
    )
    def test_excel_model(self):
        ...
        for i in range(4):
            if calculate:
                ...
                msg = (
                    "%sCalculated excel-model in %.2fs.\n%s"
            ...
            if i == 2 and platform.python_version() >= "3.8":
                ...
                print("%sFinished JSON excel-model in %.2fs." % (_msg, time.time() - s))
                calculate = True
        ...
        n_test += self._compare(
            ...
            actual=_convert_complex(
                _file2books(
                    *(osp.join(dirpath, fp) for fp in xl_mdl.books), _raw_data=True
                )
            ),
        )
    ...
    def test_excel_model_full_range(self):
        ...
        xl_model.complete([f"'test_files/[{fname}]DATA'!B5"]).finish(complete=False)
    ...
    def test_excel_from_dict(self):
        xl_model = (
            ExcelModel()
            .from_dict({"A1": 1, "B2": "=R[-1]C[-1]", "A": 2, "B": "=2", "C": "=A1"})
            .finish()
        )
        self.assertEqual(
            {"A1": 2, "B2": 2, "A": 2, "B": 2, "C": 2},
            {
        ...
        self.assertEqual(
            {
                "A1:A8": (1, 2, 3, 4, 5, 6, 7, 8),
                sh.SELF: xl_model.dsp,
                "A8": (8,),
                "A6": (6,),
                "A1": (1,),
                "A2": (2,),
